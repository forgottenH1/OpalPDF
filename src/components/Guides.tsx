...
export default function Guides({ externalSearch }: GuidesProps) {
    ...
    // Standardize IDs and fallbacks
    const activeGuide = guideId ? toolsGuides.find(g => g.id.toLowerCase() === guideId) || null : null;

    // Force redirect from legacy /guides/split to /guides/split-pdf for URL cleanliness
    React.useEffect(() => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredGuides.map(guide => (
                    ...
                    >
                        ...
                        <h3 className="text-base md:text-xl font-bold text-slate-100 mb-0.5 md:mb-3 group-hover:text-blue-400 transition-colors text-left">
                            {guide.title}
                        </h3>
                        <p className="text-slate-400 text-[10px] md:text-sm mb-0 md:mb-6 flex-grow line-clamp-2 md:line-clamp-3 text-left">
                            {guide.intro}
                        </p>
